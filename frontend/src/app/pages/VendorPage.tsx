    ...
    <div className="space-y-6 max-w-[1600px] mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card className="p-5 flex flex-col justify-between border-none shadow-sm bg-white hover:shadow-md transition-shadow relative overflow-hidden">
          ...
          <div className="text-xs uppercase font-bold text-gray-400 mb-2 tracking-wider">Late Rate</div>
          <div className="flex items-baseline gap-2">
            <div className="text-3xl font-black text-gray-900 tracking-tight">{latePayments?.latePaymentPercentage.toFixed(1)}%</div>
          </div>
          <div className="text-[11px] font-bold text-[#f43f5e] mt-2 truncate w-full">Overdue Payments</div>
        </Card>

        {/* Top Score KPI */}
        <Card className="p-5 flex flex-col justify-between border-none shadow-sm bg-white hover:shadow-md transition-shadow relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-[#8b5cf6]" />
          <div className="text-xs uppercase font-bold text-gray-400 mb-2 tracking-wider">Top Score</div>
          <div className="flex items-baseline gap-2">
            <div className="text-3xl font-black text-gray-900 tracking-tight">{vendorScores[0]?.vendorScore.toFixed(1) || "0.0"}</div>
          </div>
          <div className="text-[11px] font-bold text-[#8b5cf6] mt-2 truncate w-full" title={vendorScores[0]?.vendorName}>{vendorScores[0]?.vendorName || "N/A"}</div>
        </Card>

        {/* Top Profit Contributor KPI */}
        <Card className="p-5 flex flex-col justify-between border-none shadow-sm bg-white hover:shadow-md transition-shadow relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1 h-full bg-[#10b981]" />
          <div className="text-xs uppercase font-bold text-gray-400 mb-2 tracking-wider">Top Profit Vendor</div>
          <div className="flex items-baseline gap-2">
            <div className="text-3xl font-black text-gray-900 tracking-tight">{formatCurrency(profitContributions[0]?.totalProfit || 0)}</div>
          </div>
          <div className="text-[11px] font-bold text-[#10b981] mt-2 truncate w-full" title={profitContributions[0]?.vendorName}>{profitContributions[0]?.vendorName || "N/A"}</div>
        </Card>
          ...
          <div className="relative">
            <div className="h-6 bg-gray-100 rounded-full overflow-hidden mb-3">
              <div
                ...
                style={{ width: paymentDelay ? `${Math.min(100, (paymentDelay.averageDelayDays / 30) * 100)}%` : "0%" }}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={profitContributions} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
              <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
              <XAxis dataKey="vendorName" fontSize={10} />
              <YAxis fontSize={10} tickFormatter={(v) => formatCurrency(v)} />
              <Tooltip formatter={(v: number) => [formatCurrency(v), ""]} />
              <Bar dataKey="totalRevenue" name="Revenue" fill="#94a3b8" radius={[4, 4, 0, 0]} />
              ...
              <div key={index} className="flex items-center gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
                ...
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <span className="text-sm font-black text-[#06b6d4]">{score.vendorScore.toFixed(1)}</span>
                  </div>
                  <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-[#06b6d4] to-[#3b82f6]"
                      style={{ width: `${Math.min(100, (score.vendorScore / (vendorScores[0]?.vendorScore || 1)) * 100)}%` }}
                    />
                  </div>
                </div>
              </div>
