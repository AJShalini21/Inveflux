...
import { operationalService, ReorderAlert, TopProduct, CategoryPerformance, SalesTrend } from "../services/operational";
...
export function OperationalPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [alertsData, topProductsData, categoryData, trendData, perfData, payablesData] = await Promise.all([
  ...
  const vendorGroups = alerts.reduce((acc, alert) => {
  ...
  const getRiskColor = (type: string) => {
    switch (type) {
      case 'RED': return 'bg-[#f43f5e]';
      case 'YELLOW': return 'bg-[#f97316]';
      case 'BLUE': return 'bg-[#06b6d4]'; // Overstock cyan
      default: return 'bg-[#84cc16]';
    }
  };
  ...
  const getAlertLabel = (level: string) => {
    switch (level) {
      ...
      case 'YELLOW': return 'Low Stock';
      case 'BLUE': return 'Overstocked';
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Risk Summary</h2>
          </div>
          <RiskSummaryPanel
        ...
        <div className="lg:col-span-9 space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-bold text-gray-400 uppercase tracking-widest">Product Risk Breakdown</h2>
            <div className="flex gap-4 text-xs font-medium text-slate-500 mt-1">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#f43f5e]" /> Critical</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#f97316]" /> Caution</span>
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-[#06b6d4]" /> Excess</span>
            </div>
          </div>
          ...
          <div className="flex flex-col overflow-y-auto max-h-[800px] pr-2 custom-scrollbar">
            {Object.keys(vendorGroups).length > 0 ? (
              ...
              <div className="h-64 flex flex-col items-center justify-center bg-white border border-dashed border-gray-200 rounded-2xl text-gray-400 space-y-3">
                <p className="text-sm font-medium">{loading ? "Analyzing vendor data..." : "No inventory risks detected across vendors"}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        ...
        <Card className="p-6 flex flex-col justify-center col-span-1 border-none shadow-sm bg-white">
          <div className="text-xs text-gray-400 uppercase tracking-widest font-bold mb-3">Total Vendor Payables</div>
          <div className="text-4xl font-light mb-2 text-[#f43f5e]">
        ...
        <Card className="p-6 col-span-1 lg:col-span-3 border-none shadow-sm bg-white">
          <h3 className="text-base font-black text-gray-900 mb-4 tracking-tight">VENDOR PERFORMANCE</h3>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="border-b border-slate-100 bg-slate-50/50">
                <tr>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider rounded-tl-lg">Vendor Name</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Payables</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider">Avg Payment Delay</th>
                  <th className="text-left py-3 px-4 text-xs font-semibold text-slate-500 uppercase tracking-wider rounded-tr-lg">% Late Payments</th>
                </tr>
              </thead>
              <tbody>
                {performanceData.map((vendor, index) => (
                  <tr key={index} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-slate-600">{formatCurrency(vendor.totalPayables)}</td>
                    <td className={`py-3 px-4 font-bold ${vendor.avgPaymentDelay > 15 ? "text-[#f43f5e]" : vendor.avgPaymentDelay > 5 ? "text-[#f97316]" : "text-[#84cc16]"}`}>
                      {vendor.avgPaymentDelay.toFixed(1)} Days
                    </td>
                    <td className="py-3 px-4">
                      <span className={`px-2.5 py-1 rounded-full text-[11px] font-bold tracking-wide ${vendor.latePaymentPercentage > 10 ? "bg-rose-50 text-[#f43f5e]" : vendor.latePaymentPercentage > 0 ? "bg-orange-50 text-[#f97316]" : "bg-lime-50 text-[#84cc16]"}`}>
