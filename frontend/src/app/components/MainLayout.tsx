...
import { Search, Bell, User, BarChart3, Package, DollarSign, Truck, Upload, Settings, Menu } from "lucide-react";
      ...
      {navItems.map((item) => {
        ...
        return (
          <Link
            key={item.path}
            to={item.path}
            className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all ${isActive(item.path)
        ...
        <div className="flex items-center justify-between gap-4">
              ...
              <div className="md:hidden">
                <Sheet>
                  <SheetTrigger asChild>
                    <Button variant="ghost" size="icon" className="rounded-xl bg-white/20 backdrop-blur-md hover:bg-white/30 border border-white/20">
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`flex items-center gap-2 px-6 py-2 rounded-xl transition-all h-full ${isActive(item.path)
          ...
          <div className="flex-1 flex items-center justify-end gap-2">
            <div className="flex items-center gap-1 sm:gap-2">
              <Button variant="ghost" size="icon" className="relative rounded-xl h-9 w-9 bg-white/90 border border-white/20">
                <Bell className="w-5 h-5 text-gray-600" />
                <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-red-500 rounded-full border border-white"></span>
              </Button>

              <Button variant="ghost" size="icon" className="hidden sm:inline-flex rounded-xl h-9 w-9 bg-white/90 border border-white/20">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="ghost" className="gap-2 rounded-xl h-9 px-1 sm:px-2 backdrop-blur-md ">
                    <Avatar className="w-7 h-7">
                      <AvatarFallback className="text-sm" style={{ background: 'var(--gradient-accent)', color: 'white' }}>
                        JD
                      </AvatarFallback>
                    </Avatar>
                  </Button>
