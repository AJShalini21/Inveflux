      ...
      <Table>
        <TableHeader className="bg-gray-50/95 sticky top-0 z-20 backdrop-blur-sm border-b border-gray-100 shadow-[0_1px_2px_rgba(0,0,0,0.02)]">
          <TableRow className="hover:bg-transparent border-none">
            <TableHead className="font-semibold text-gray-500 uppercase tracking-wider text-[11px] w-[35%] py-4">Vendor</TableHead>
            <TableHead className="font-semibold text-gray-500 uppercase tracking-wider text-[11px] py-4">Total Risks</TableHead>
            <TableHead className="font-semibold text-gray-500 uppercase tracking-wider text-[11px] py-4">Risk Breakdown</TableHead>
        ...
        <TableBody>
          {Object.entries(vendorGroups)
            ...
            const sortedAlerts = [...alerts].sort((a, b) => a.stockCoverageDays - b.stockCoverageDays);

            return (
              <Fragment key={vendor}>
                <TableRow className={`transition-colors ${isGlobalExpanded ? "bg-slate-50/30 border-b-transparent" : "hover:bg-slate-50 border-b-slate-100"}`}>
                  <TableCell className="py-4 align-middle">
                    <span className="font-bold text-slate-800 tracking-tight">{vendor}</span>
                  </TableCell>
                  <TableCell className="py-4 align-middle">
                    <span className="font-black text-slate-700 tracking-tight bg-slate-100 hover:bg-slate-200 transition-colors px-3 py-1.5 rounded-lg text-[11px] shadow-sm ring-1 ring-slate-200/50">{alerts.length}</span>
                  </TableCell>
                  <TableCell className="py-4 align-middle">
                              ...
                              <Table>
                                <TableHeader className="bg-slate-100/50">
                                  <TableRow className="border-none">
                                    <TableHead className="py-2 px-4 text-[10px] uppercase font-bold text-slate-400 tracking-wider">ID</TableHead>
                                    <TableHead className="py-2 px-4 text-[10px] uppercase font-bold text-slate-400 tracking-wider">Product Name</TableHead>
                                    <TableHead className="py-2 px-4 text-[10px] uppercase font-bold text-slate-400 tracking-wider">Risk Type</TableHead>
                                    <TableHead className="text-right py-2 px-4 text-[10px] uppercase font-bold text-slate-400 tracking-wider">Coverage</TableHead>
                                  </TableRow>
                                </TableHeader>
                                <TableBody>
                                  {sortedAlerts.map((alert) => (
                                    <TableRow key={alert.productId} className="border-slate-50 hover:bg-slate-50/80 transition-colors">
                                      <TableCell className="py-2 px-4 text-[11px] font-mono text-slate-400 tracking-tight">#{alert.productId}</TableCell>
                                      <TableCell className="py-2 px-4 font-semibold text-[12px] text-slate-700 tracking-tight">{alert.productName}</TableCell>
                                      <TableCell className="py-2 px-4 flex items-center gap-1.5 h-full">
                                        <Badge className={`${getRiskColor(alert.alertLevel)} text-white border-none shadow-sm text-[9px] px-2 py-0.5 rounded-full font-bold tracking-widest`}>
